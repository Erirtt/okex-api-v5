...
# -*- coding: utf-8 -*-
"""
backfill_history_candles_1h.py

使用 OKX REST API `/api/v5/market/history-candles` 回填最近三个月的 1H K 线：
- bar=1H
- 分页参数 after（请求此时间戳之前的数据）
- 只保存已完结（confirm=="1"）的 K 线
- 并发最多 CONCURRENCY 个交易对
- 支持重试 & 限速保护
- 写入 PostgreSQL 表 kline_1h
"""
...
logger = logging.getLogger("backfill_1h")
...
async def fetch_chunk(session: aiohttp.ClientSession, inst_id: str, after_ts: int) -> list:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if code == "50011":  # Too Many Requests
            ...
            logger.warning(f"{inst_id} rate limited, sleeping {wait}s")
...
async def backfill_inst(pool: asyncpg.Pool, inst_id: str, since_ts: int):
    logger.info(f">>> [{inst_id}] backfill start")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        while after > since_ts:
            ...
            total += len(records)
            # 下一页：设置 after 为本批最后一条 K 线 ts - 1ms
            after = int(batch[-1][0]) - 1

    logger.info(f"<<< [{inst_id}] backfill done, wrote {total} bars")
...
async def main():
    # 回填起点：3 个月前的 0 点
    since_dt = (datetime.utcnow() - timedelta(days=90)).replace(hour=0, minute=0, second=0, microsecond=0)
    since_ts = int(since_dt.timestamp() * 1000)
    logger.info(f"== Backfill history-candles 1H since {since_dt.isoformat()} ==")

    pool = await make_pool()
    try:
        # 从 instruments 表读取所有 USDT 合约
        rows = await pool.fetch("SELECT inst_id FROM instruments WHERE inst_id LIKE '%-USDT'")
        symbols = [r["inst_id"] for r in rows]
        logger.info(f"Found {len(symbols)} USDT symbols to backfill")
        ...
        async def worker(sym: str):
            async with sem:
                try:
                    ...
                    logger.error(f"{sym} ▶️ backfill error: {e}")
